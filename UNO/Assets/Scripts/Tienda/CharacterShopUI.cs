...
public class CharacterShopUI : MonoBehaviour
{
	...
	{	
		...
		Debug.Log("Elementos lista" + GameDataManager.GetAllPurchasedCharacter ().Count);
		
		//Delete itemTemplate after calculating item's Height :
		itemHeight = ShopItemsContainer.GetChild (0).GetComponent <RectTransform> ().sizeDelta.x;
		...
		for (int i = 0; i < characterDB.CharactersCount; i++) {
			...
			uiItem.SetBackgroundImage (character.imageBackground);
			uiItem.SetCharacterPrice (character.price);

			if (character.isPurchased) {
				//Character is Purchased
				uiItem.SetCharacterAsPurchased (); // pone bool purchased = true
			...
			ShopItemsContainer.GetComponent <RectTransform> ().sizeDelta = 
				Vector2.right * ((itemHeight + itemSpacing) * characterDB.CharactersCount-1);
	...
	void OnItemPurchased (int index)
	{
		...
		if (GameDataManager.CanSpendCoins (character.price)) {
			...
			//Update DB's Data
			characterDB.PurchaseCharacter (index); //con esto ponemos el tick de la bbdd a true
			...
			//Actualiza el perfil
			PerfilManager pf = GameObject.Find("PerfilManagers").GetComponent<PerfilManager>();
			pf.Start();
